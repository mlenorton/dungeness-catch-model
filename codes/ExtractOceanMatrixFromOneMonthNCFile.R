ExtractOceanMatrixFromOneMonthNCFile<-function(thisMonthsNCFileName)
{
  ...
  # returns oceanMatrix for one month, after extracing from NC ROMS file. 
...
yearROMSstart <-as.numeric(str_sub(thisMonthsNCFileName,-23,-20))
monthROMSString  <-str_sub(thisMonthsNCFileName,-16,-14)

monthAbbreviations<-c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
monthROMSOceanConditions <- which(monthAbbreviations ==monthROMSString  )

#----------
# Open a connection to this File
#------------

nc <- nc_open(thisMonthsNCFileName)

#-----------

# Save the print(nc) dump to a text file (same name as the nc file with a txt extension)
{
  sink(paste0(thisMonthsNCFileName, ".txt"))
...
oceanMatrix <- cbind(nc_latVector,nc_lonVector,bathymetryVector,bottomTempVector,bottomOxyVector,bottomSaltVector,SSHVector,Chla2mVector,bottomAragVector,bottompHVector,yearROMSstart,monthROMSOceanConditions)
